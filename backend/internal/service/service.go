...
func NewService(c *cache.ShardedCache) *Service {
	...
	s.detectors = []detector{
		...
		{re: regexp.MustCompile(`(?i)серия\D{0,4}(\d{2}\s*\d{2})\D{0,10}(\d{6})\b`), typ: "PASSPORT", groups: []int{1, 2}},
		{re: regexp.MustCompile(`(?i)(?:номер|№)\s*(\d{6})\b`), typ: "PASSPORT", groups: []int{1}},
		{re: regexp.MustCompile(`\b\d{3}-\d{3}\b`), typ: "PASS_CODE"},
		// Орган, выдавший паспорт
		{re: regexp.MustCompile(`(?i)((?:ОВД|УФМС|ОУФМС|ГУ\s?МВД|УМВД|ГУВД|ТП|МО\s?УФМС|отдел(?:ом|ением)?)[^\n,.;]{0,60})`), typ: "PASS_AUTHORITY", groups: []int{1}},

		// Даты: числовые (любой порядок) и текстовые
