...
export async function getConfig():Promise<any>{const r=await fetch('/config');if(!r.ok)throw new Error('config load failed');return r.json()}
export async function putConfig(cfg:any):Promise<boolean>{const r=await fetch('/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(cfg)});return r.ok}
